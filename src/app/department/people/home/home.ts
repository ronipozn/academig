...
@Component({
  selector: 'department-people',
  templateUrl: 'home.html',
  styleUrls: ['home.css']
})
export class DepartmentPeopleComponent implements OnInit {
  people: People[][] = [[],[],[],[],[]];
  ...
  ngOnInit() {
    if (this.missionService.departmentId) {
      this.titleService.setTitle('People - ' + this.missionService.departmentTitle + ' | Academig');
      this.activeTab = 0;
    }
  ...
  async peopleFunc(tabNum: number) {
    ...
    if (this.streamRetrieved[tabNum] == false) {
      ...
      const people = await this.peopleService.getPeoples(this.missionService.searchFlag ? 15 : 11, this.missionService.departmentId, null, tabNum, 0, this.more[this.activeTab], this.missionService.searchText);
