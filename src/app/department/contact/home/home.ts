...
export class DepartmentContactComponent {
  ...
  ngOnInit() {
    ...
    this.titleService.setTitle('Contact - ' + this.missionService.departmentTitle + ' | Academig');
    this.updatePage()
  }

  async updatePage() {
    if (this.missionService.departmentId) {
      this.items = await this.departmentService.getContactsPageByDepartmentId(this.missionService.departmentId);
      this.streamRetrieved[1] = true;

      this.contacts = await this.contactService.getContacts(1, this.missionService.departmentId);
  ...
  async contactUpdate(event) {
    ...
    if (this.contactNewFlag == true) {
      ...
      this.contacts[this.contacts.length - 1]._id = await this.contactService.putContact(1, contact, this.missionService.departmentId);
      ...
      await this.contactService.postContact(1, contact, this.contacts[this.contactIndex]._id, this.missionService.departmentId);
  ...
  async contactDelete(i: number) {
    ...
    await this.contactService.deleteContact(1, _id, this.missionService.departmentId);
  ...
  async findUsOp(mode: number, flag: boolean, event) {
    ...
    if (mode == 1) {
      ...
      await this.sharedService.deleteTextPic(4, this.missionService.departmentId, this.missionService.departmentId);
      ...
      await this.sharedService.postTextPic(4, this.missionService.departmentId, this.missionService.departmentId, event.text, event.pic, event.caption);
