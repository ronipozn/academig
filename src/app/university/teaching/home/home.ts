...
@Component({
  selector: 'university-teaching',
  templateUrl: 'home.html',
  styleUrls: ['home.css']
})
export class UniversityTeachingComponent implements OnInit {
  ...
  updatePage() {
    if (this.missionService.universityId) {
      this.titleService.setTitle('Teaching - ' + this.missionService.universityName + ' | Academig');
    }
  }
  ...
  async findTeachings() {
    ...
    const teachings = await this.teachingService.getTeachings(this.missionService.searchFlag ? 7 : 6, this.missionService.universityId, 0, this.missionService.searchText);
