...
@Component({
  selector: 'university-media',
  templateUrl: 'home.html',
  styleUrls: ['home.css']
})
export class UniversityMediaComponent implements OnInit {
  talks: Talk[] = [];
  posters: Poster[] = [];
  presses: Press[] = [];
  ...
  ngOnInit() {
    if (this.missionService.universityId) {
      this.titleService.setTitle('Media - ' + this.missionService.universityName + ' | Academig');
      this.activeTab = 0;
    }
  ...
  async mediaFunc(tabNum: number) {
    ...
    if (this.streamRetrieved[tabNum] == false) {
      ...
      const data = await this.mediaService.getMedia(this.missionService.searchFlag ? 6 : 5, this.missionService.universityId, tabNum, 0, this.more[this.activeTab], this.missionService.searchText);
