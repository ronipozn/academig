...
@Component({
  selector: 'university-resources',
  templateUrl: 'home.html',
  styleUrls: ['home.css']
})
export class UniversityResourcesComponent implements OnInit {
  stream: number[];
  streamFollow: number[];
  ...
  ngOnInit() {
    if (this.missionService.universityId) {
      this.titleService.setTitle('Services - ' + this.missionService.universityName + ' | Academig');
    }
  ...
  async findResources() {
    ...
    const resources = await this.resourceService.getResources<Resource>(this.missionService.searchFlag ? 10 : 9, this.missionService.universityId, null, this.more, this.missionService.searchText);
