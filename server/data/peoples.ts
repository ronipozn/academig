...
const algoliaIndex = process.env.PORT
  ? client.initIndex("researchers")
  : client.initIndex("dev_researchers");
    ...
    [
      ...
      function (people_data, cb) {
        ...
        createPeopleDocument(
          0,
          people_data.email_verified ? 2 : 1,
          data_clone,
  ...
  async.series(
    {
      ...
      enable: function (cb) {
        db.peoples
          ...
          .then(function (item) {
            if (
              userId == "academig" ||
              item.positions.filter((r) => r.status == 6).length > 0
            ) {
              console.log(
                "PI must delete all his labs before deleting himself"
              );
            ...
            async.parallel(
              {
                positions: function (cb) {
                  async.forEachOf(
                    item.positions,
                    function (position, key, callback) {
                      groups.delete_member(
                        ...
                        function (err) {
                          ...
                          async.forEachOf(
                            item.contactsIds,
                            function (contactId, key, callback) {
                ...
                publications: function (cb) {
                  if (item.publicationsIds) {
                    publications.delete_publications(
                      userId,
                      null,
                      3,
                      item.publicationsIds,
                      function (err) {
                        console.log("2");
                        cb();
                      }
                ...
                posters: function (cb) {
                  async.forEachOf(
                    item.postersIds,
                    function (posterId, key, callback) {
                      media.delete_media(
                        userId,
                        posterId,
                        2,
                ...
                reports: function (cb) {
                  async.forEachOf(
                    item.reportsIds,
                    function (reportId, key, callback) {
                      user_settings.pull_report(
                        userId,
                        reportId,
                        function (err) {
                          callback();
                ...
                publicationsFollowings: function (cb) {
                  if (item.followings.publicationsIds) {
                    delete_followed_ids(
                      "publications",
                      userId,
                      item.followings.publicationsIds,
                      function (err) {
                        console.log("12");
                        cb(err);
                      }
                ...
                oAuth: function (cb) {
                  request(authOptions, function (error, response, body) {
                    ...
                    var options = {
                      ...
                      url:
                        "https://academig.eu.auth0.com/api/v2/users/" +
                        item.oauth_id,
      ...
      people: function (cb) {
        console.log("23");
        db.peoples.deleteOne({ _id: userId }, { safe: true }, cb());
      },
...
// MOVE
export function update_auth_metadata(
  userId: string,
  type: number,
  itemId: string,
  callback
    ...
    .then(function (item) {
      request(authOptions, function (error, response, body) {
        ...
        var options = {
          ...
          headers: { authorization: "Bearer " + access_token },
          body: { app_metadata: { privilages: type } },
          json: true,
        };
    ...
    [
      ...
      function (position_data, cb) {
        ...
        if (existMemberFlag) {
          ...
          // part of creating a new Group Member
          db.peoples.updateOne(
            { _id: peopleId },
            {
              $push: {
  ...
  async.series(
    {
      ...
      position: function (cb) {
        db.peoples.updateOne(
          {
            _id: people_data._id,
            "positions._id": ObjectID(people_data.positionId),
          },
          {
            $set: {
            ...
            .updateOne(
              { _id: ObjectID(groupId) },
              {
                $inc: {
                  ...
                  memberType +
                  "." +
                  people_data.oldCategory]: -1,
                  ["peoplesPageItems." +
                  memberType +
                  "." +
                  people_data.newCategory]: 1,
                },
...
export function update_dummy_position(people_data, callback) {
  async.waterfall(
    [
      ...
      function (cb) {
        if (
          people_data.titles[0] == 300 ||
          people_data.titles[0] == 301 ||
          people_data.titles[0] == 303
        ) {
          db.peoples
            .find({
              _id: people_data._id,
        ...
        news.add_activity(
          ...
          function (err) {
            groups.groupMembers(groupId, 3, function (err, actives) {
              async.forEachOf(
                actives.map((r) => r._id),
                function (activeId, key, callback) {
                  if (userId == activeId) {
                    callback();
                  } else {
...
export function people_typeahead_ids(
  term: string,
  objId: string,
  type: number,
  callback
  ...
  if (type < 2) {
    ...
    db[collection]
      ...
      .then(function (items) {
        if (type == 0 && items) {
          negateIds = items.peoplesItems.activesIds.concat(
            items.peoplesItems.alumniIds
          );
...
export function createPeopleDocument(
  mode: number,
  stage: number,
  data,
  callback
    ...
    [
      ...
      function (groupId: string, cb) {
        groups.get_group_link(groupId, function (err, item) {
          var groupLink =
            item.groupIndex.university.link +
            "/" +
            item.groupIndex.department.link +
            "/" +
            item.groupIndex.group.link;
          cb(err, groupLink);
        });
      },
    ],
...
export function get_followings_ids(
  mode: number,
  peopleId: string,
  type: number,
  callback
  ...
  if (!peopleId) {
    ...
    db.peoples
      ...
      .project({
        ...
        publicationsIds: 1,
        publicationsSuggestionsIds: 1,
        publicationsRejectedIds: 1,
        talksIds: 1,
        postersIds: 1,
        pressesIds: 1,
        fundingsIds: 1,
      ...
      .then(function (items) {
        switch (mode) {
          case 0:
            callback(null, items.followings.publicationsIds);
            break;
          case 1:
            callback(null, items.followings.resourcesIds);
            break;
          case 2:
            callback(null, items.followings.projectsIds);
            break;
          case 3:
            callback(null, items.followings.positionsIds);
            break;
          case 4:
            callback(null, items.followings.groupsIds);
            break;
          case 5:
            switch (type) {
              case 0:
                callback(null, items.groupsRelations.activesIds);
                break;
              case 1:
                callback(null, items.groupsRelations.alumniIds);
                break;
              case 2:
                callback(null, items.groupsRelations.visitorsIds);
                break;
            }
            break;
          case 6:
            callback(null, items.projectsIds);
            break;
          case 7:
          ...
          case 8:
            switch (type) {
              case 0:
                callback(null, items.talksIds);
                break;
              case 1:
                callback(null, items.postersIds);
                break;
              case 2:
                callback(null, items.pressesIds);
                break;
            }
            break;
          case 9:
            callback(null, items.followings.peoplesIds);
            break;
          case 10:
            callback(null, items.fundingsIds);
            break;
          case 11:
            callback(null, items.blockingIds);
            break;
          case 12:
            callback(null, items.blockedIds);
            break;
          case 13:
            callback(null, items.resourcesIds);
            break;
          case 14:
            callback(null, items.profileProjectsIds);
            break;
          // case 15: callback(null, items.profileResourcesIds); break; // DELETED
          case 16:
            callback(null, items.teachingsIds);
            break;
          case 17:
            callback(null, items.publicationsSuggestionsIds);
            break;
          case 18:
            callback(
              null,
              items.publicationsIds.concat(items.publicationsRejectedIds)
            );
            break;
          case 19:
            callback(null, items.galleriesIds);
            break;
          case 20:
            callback(null, items.publicationsRejectedIds);
            break;
          case 21:
            callback(null, items.outreachsIds);
            break;
          case 30:
            callback(null, items.requests);
            break;
          case 40:
            callback(null, items.followings.podcastsIds);
            break;
          case 50:
            callback(null, items.followings.eventsIds);
            break;
          case 60:
            callback(null, items.followings.appsIds);
            break;
          case 70:
            callback(null, items.mentorsIds);
            break;
          case 80:
            callback(null, items.dealsIds);
            break;
        }
      });
  }
...
export function get_library_ids(userId: string, peopleId: string, callback) {
  const f =
    userId == peopleId
      ...
      : {
          _id: peopleId,
          $or: [{ libraryPrivacy: 0 }, { libraryPrivacy: { $exists: false } }],
        };
    ...
    db.peoples
      ...
      .then(function (items) {
        ...
        const filteredIds = items
          .map((r) => r._id)
          .filter((e) => filterIds.indexOf(e) < 0)
          .slice(0, 3);
...
export function set_library_folder(
  peopleId: string,
  itemId: string,
  folderObj: any,
  callback
  ...
  if (
    folderObj.folder == "read" ||
    folderObj.folder == "current" ||
    folderObj.folder == "want"
  ) {
    //  ||

    const libraryPeoplesPull = Object.assign(
      folderObj.folder != "current"
        ? { ["library.current"]: ObjectID(itemId) }
        : {},
      ...
      .then(function (item) {
        db.publications.updateOne(
          { _id: ObjectID(itemId) },
          {
            $pull: {
              ["folders." + peopleId]: {
                folder: { $in: libraryPublicationsPull },
              },
            },
          },
          { safe: true }
      ...
      .then(function (item) {
        news_library_folder(
          ...
          function (err) {
            callback(
              err,
              folderObj.folder == "read" ? publicationObj._id : null
            );
          }
        );
      });
...
export function post_read_folder(
  peopleId: string,
  itemId: string,
  folderObj: any,
  callback
  ...
  db.publications
    .updateOne(
      {
        _id: ObjectID(itemId),
        ["folders." + peopleId + "._id"]: ObjectID(folderObj._id),
      },
...
export function pull_library_folder(
  peopleId: string,
  itemId: string,
  folder: string,
  callback
...
export function pull_read_folder(
  peopleId: string,
  itemId: string,
  folderId: string,
  callback
...
export function unset_library_folder(
  peopleId: string,
  folder: string,
  callback
) {
...
export function news_library_folder(
  putFlag: boolean,
  peopleId: string,
  itemId: string,
  folder: string,
  ...
  if (putFlag == true) {
    ...
    news.follow_feed(itemId, peopleId, 0, 1, function (err) {
      // mode==0
      callback(err);
    });
  }
...
export function toggle_followings_ids(
  putFlag: boolean,
  mode: number,
  peopleId: string,
  type: number,
  ...
  if (!peopleId) {
    ...
    switch (mode) {
      ...
      case 5: {
        ...
        switch (type) {
          case 0:
            key = "groupsRelations.activesIds";
            break;
          case 1:
            key = "groupsRelations.alumniIds";
            break;
          case 2:
            key = "groupsRelations.visitorsIds";
            break;
        }
        break;
      }
...
export function toggle_blocking_ids(
  putFlag: boolean,
  userId: string,
  peopleId: string,
  callback
...
export function toggle_compare_ids(
  putFlag: boolean,
  userId: string,
  itemId: string,
  callback
...
export function delete_followings_ids(
  collection: string,
  key: string,
  itemId: string,
  mode: boolean,
...
export function delete_followed_ids(
  collection: string,
  userId: string,
  itemsIds,
  callback
...
export function peoples_emails(peoplesIds: string[], callback) {
  ...
  var f = {
    $project: { _id: 0, stage: 1, name: 1, pic: 1, "personalInfo.email": 1 },
  };
...
export function peoples_list(
  ...
  if (mini == 0) {
  ...
  } else if (mini == 2 || mini == 6) {
    // 2: Add Position, Add Role, ...
    ...
    // People Admin, Group Markeitng, Delete Member
...
export function peoples_list_department(
  peoplesIds: string[],
  groupsIds: string[],
  followingsIds: string[],
  callback
...
export function people_actives(
  _id: string,
  mode: boolean = true,
  statusLte: number,
  callback
  ...
  async.waterfall(
    [
                ...
                function (err, items) {
                  people.positions.forEach(function (position) {
                    position.contest = contest
                      ? contest.groupsIds.find(
                          (id) => id.toString() == position.groupId.toString()
                        )
                        ? contestDetails
                        : null
                      : null;
                    const item = items.find(
                      (item) =>
                        item._id.toString() == position.groupId.toString()
                    );
              ...
              people.invites.forEach(function (invite) {
                if (invite.mode < 4) {
                  groupItem = items.find(
                    (item) => item._id.toString() == invite.groupId.toString()
                  );
      ...
      function (people, cb) {
        ...
        if (people && people.invites && people.invites[0]) {
          var publicationsIds = people.invites
            .filter((r) => r.mode == 4)
          ...
          publications.publications_list(
            ...
            function (err, items) {
              people.invites.forEach(function (invite) {
                if (invite.mode == 4) {
                  publicationItem = items.find(
                    (item) =>
                      item._id.toString() == invite.publicationId.toString()
                  );
...
export function people_group_relations(peopleId: string, callback) {
  db.peoples
    ...
    .project({ _id: 0, positions: 1, visits: 1 })
    .next(callback);
}
...
export function people_profile(peopleId: string, adminFlag: boolean, callback) {
  ...
  const p = {
    ...
    meetClip: 1,
    researchInterests: 1,
    recreationalInterests: 1, // progress: 1,
  ...
  db.peoples.aggregate([{ $match: m }, { $project: p }]).next(callback);
}

export function people_profile_basic(
  peopleId: string,
  followingsIds: string[],
  blockingsIds: string[],
  groupId: string,
  ...
  if (followingsIds || blockingsIds) {
    db.peoples
      .aggregate([{ $match: m }, { $project: p }])
      .map(function (u) {
        if (followingsIds)
          u.followStatus = followingsIds.toString().includes(u._id.toString());
        if (blockingsIds)
          u.blockStatus = blockingsIds.toString().includes(u._id.toString());
        return u;
      })
      .next(callback);
  } else {
...
export function people_news_updtes(peopleId: string, callback) {
  db.peoples
    .find({ _id: peopleId })
    .project({ _id: 1, name: 1, pic: 1, positions: 1, country: 1 })
    .next(callback);
}
...
export function post_user_name(name: string, userId: string, callback) {
  db.peoples
    .updateOne({ _id: userId }, { $set: { name: name } }, { safe: true })
    .then(function (item) {
...
export function post_people_mini(
  peopleId: string,
  name: string,
  pic: string,
  mode: number,
            ...
            async.parallel(
              {
                ...
                publications: function (cb) {
                  if (item.publicationsIds) {
                    db.publications.updateMany(
                      {
                        _id: {
                          $in: item.publicationsIds.map((r) => ObjectID(r)),
                        },
                        "authors._id": peopleId,
                      },
                      {
                    ...
                    db.projects.updateMany(
                      {
                        _id: {
                          $in: item.projectsIds
                            .concat(item.profileProjectsIds)
                            .map((r) => ObjectID(r)),
                        },
                        "people._id": peopleId,
                      },
                ...
                fundings: function (cb) {
                  if (item.fundingsIds) {
                    db.fundings.updateMany(
                      {
                        _id: { $in: item.fundingsIds.map((r) => ObjectID(r)) },
                        "roles.member._id": peopleId,
                      },
                      {
                        $set: {
                          ["roles.$.member." + key]: mode == 0 ? name : pic,
                        },
                      },
                      { multi: true, safe: true },
                      cb()
                ...
                media: function (cb) {
                  if (item.mediaIds) {
                    db.media.updateMany(
                      {
                        _id: { $in: item.mediaIds.map((r) => ObjectID(r)) },
                        "presentors._id": peopleId,
                      },
                      {
                        $set: {
                          ["presentors.$." + key]: mode == 0 ? name : pic,
                        },
                      },
                      { multi: true, safe: true },
                      cb()
                ...
                groups: function (cb) {
                  if (item.positions) {
                    db.groups.updateMany(
                      {
                        _id: {
                          $in: item.positions.map((r) => ObjectID(r.groupId)),
                        },
                        $or: [
                          {
                            "futureMeetingsItems.settings.participants._id":
                              peopleId,
                          },
                          {
                            "futureMeetingsItems.meetings.presenter._id":
                              peopleId,
                          },
                          {
                            "pastMeetingsItems.meetings.presenter._id":
                              peopleId,
                          },
                          { "reportsItems.settings.whoSubmit._id": peopleId },
                          { "reportsItems.settings.whoSee._id": peopleId },
                          {
                            "reportsItems.currentReport.whoSubmit._id":
                              peopleId,
                          },
                      ...
                      {
                        $set: {
                          ["futureMeetingsItems.settings.participants.$[]." +
                          key]: mode == 0 ? name : pic,
                          ["futureMeetingsItems.meetings.$[].presenter." + key]:
                            mode == 0 ? name : pic,
                          ["pastMeetingsItems.meetings.$[].presenter." + key]:
                            mode == 0 ? name : pic,
                          ["reportsItems.settings.whoSee.$[]." + key]:
                            mode == 0 ? name : pic,
                          ["reportsItems.settings.whoSubmit.$[]." + key]:
                            mode == 0 ? name : pic,
                          ["reportsItems.currentReport.whoSee.$[]." + key]:
                            mode == 0 ? name : pic,
                          ["reportsItems.currentReport.whoSubmit.$[]." + key]:
                            mode == 0 ? name : pic,
                        },
                      },
                      { multi: true, safe: true },
...
export function delete_private_report(
  itemId: string,
  userId: string,
  callback
) {
...
export function put_share(data, mode: number, userId: string, callback) {
  ...
  async.forEachOf(
    data_clone.ids,
    function (peopleId, key, callback) {
      news.add_notification(
        userId,
        13000 + mode,
        data_clone.itemId,
...
export function verify_institute_email(
  userId: string,
  groupId: string,
  callback
) {
...
export function update_institute_email(
  userId: string,
  groupId: string,
  email: string,
  callback
...
export function put_contact_message(
  userId: string,
  toId: string,
  mode: number,
  callback
        ...
        stripe.customers.retrieve(item.stripe_id, function (err, customer) {
          ...
          customer_plan = sub
            ? {
                ...
                plan: process.env.PORT
                  ? sub.plan.id == "plan_FEkfxvASc3fxnL" ||
                    sub.plan.id == "plan_FEkgayuUC0K5g3"
                    ? 1
                    : 0
                  : sub.plan.id == "plan_FDW4rRATIUJwx3" ||
                    sub.plan.id == "plan_FDW4RZ2S8HJ1LT"
                  ? 1
                  : 0,
              }
            : null;
...
export function create_people_profile(
  host,
  userId: string,
  data: CreateProfile,
  callback
    ...
    [
        ...
        db.peoples.updateOne(
          { _id: userId },
          {
            $set: {
              ...
              coverPic: data.cover
                ? data.cover
                : "https://s3.amazonaws.com/academig/coverPics/big/" +
                  sub_pics[data.theme] +
                  "_" +
                  data.themeIndex +
                  ".jpeg",
              ...
              "followings.groupsIds": data.groupsIds
                ? data.groupsIds.map((i) => ObjectID(i))
                : [],
            },
          },
          { safe: true },
        ...
        algoliaIndex.partialUpdateObject(
          {
            ...
            country: data.country_id
              ? countries[countries.findIndex((y) => y.id == data.country_id)]
                  .name
              : null,
      ...
      function (cb) {
        ...
        if (data.university && data.department && data.group) {
          ...
          group_create_data.create_group(
            groupBuild,
            host,
            userId,
            function (err, items) {
...
export function create_dummy_position(
  userId: string,
    ...
    [
      ...
      function (cb) {
        ...
        universities.university_id(
          position.groupComplex.university.name,
          0,
      ...
      function (cb) {
        ...
        departments.department_id(
          position.groupComplex.university.name,
          position.groupComplex.department.name,
          0,
      ...
      function (cb) {
        ...
        groupComplex = {
          group: {
            _id: null,
            name: position.groupComplex.group.name,
            pic: null,
            link: null,
          }, // take only name and not link
        ...
        const dummy_obj = {
          titles:
            university.name == "company"
              ? position.titles
              : [Number(position.titles[0])],
          group: groupComplex,
          period: position.period,
        };

        groups.create_dummy_member(
          dummy_obj,
          userId,
          function (err, positionArr) {
      ...
      function (
        ...
        universities.universities_query(
          position.groupComplex.university.name,
          1,
...
export function generate_people_position(
  mode: number,
  status: number,
  period: Period,
  title: string,
...
function people_algolia_positions(peopleId: string, callback) {
  db.peoples
    ...
    .then(function (item) {
      groups.groups_list(
        item.positions.map((g) => g.groupId),
        null,
        null,
        null,
        null,
        ...
        function (err, items) {
          item.positions.forEach(function (position) {
            const item = items.find(
              (item) => item._id.toString() == position.groupId.toString()
            );
            position.group = item ? item.groupIndex : null;
            delete position.groupId;
...
function invalid_people_name() {
  return backhelp.error(
    "invalid_people_name",
    "People names can have letters, #s, _ and, -"
  );
}
