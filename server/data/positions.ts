...
export function put_position(userId: string, groupId: string, adminFlag: boolean, data, callback) {
  var data_clone;
  ...
  async.waterfall([
    ...
    function (position_data, cb) {
      ...
      groups.get_group_algolia(groupId, function (err, item) {
        cb(err, item.groupIndex, item.country, item.state, item.city, item.location, position_data)
      });
    },

    // 2B. create Position document
    function (group: groupComplex, country: string, state: string, city: string, location: number[], position_data, cb) {
      console.log('P2')
      data_clone = JSON.parse(JSON.stringify(position_data));

      createPositionDocument(data_clone, groupId, group, country, state, city, location, adminFlag, function (err, positionId) {
        cb(null, positionId)
      });
    },
    ...
    function (positionId, cb) {
      ...
      if (!userId || (adminFlag && data_clone.ai)) {
        ...
        push_news(userId, positionId, groupId, data_clone.description, data_clone.spotsAvailable, data_clone.stepsDates, function (err) {
          cb(err, positionId)
        })
      }
    },
...
export function post_position_notify(stripeId: string, subscriptionId: string, itemId: string, callback) {
  ...
  async.waterfall([
    ...
    function (cb) {
      groups.group_notify_details(stripeId, function (err, groupIndex, country, state, city, location, stage, userId, pro, interview, club) {
        cb(err, groupIndex, stage, userId, pro, interview, club);
      })
    },

    // 2. get hash associated with User ID
    function (groupIndex: groupComplex, groupStage: number, userId: string, pro: boolean, interview: boolean, club: boolean, cb) {
      db.hash
        .find({"userId": userId})
        .project({hash: 1})
      .next().then(function(item) {
        cb(null, item ? item.hash : null, groupIndex, groupStage, userId, pro, interview, club);
      });
    },

    // 3. retrieve User Email, Name
    function (hash: string, groupIndex: groupComplex, groupStage: number, userId: string, pro: boolean, interview: boolean, club: boolean, cb) {
      db.peoples
        .find({"_id": userId})
        .project({stage: 1, name: 1, "personalInfo.email": 1})
      .next().then(function(item) {
        cb(null, item.stage, item.name, item.personalInfo.email, hash, groupIndex, groupStage, userId, pro, interview, club);
      });
    },

    // 4. retrieve Position data && set payment_flag=true
    function (userStage: number, userName: string, userEmail: string, hash: string, groupIndex: groupComplex, groupStage: number, userId: string, pro: boolean, interview: boolean, club: boolean, cb) {
      ...
      db.positions.findOneAndUpdate(
        ...
        { projection:
          {
            "created_on": 1, "standout": 1, "position": 1, "title": 1, "type": 1, "description": 1,
            "spotsAvailable": 1, "contractLength": 1, "contractLengthType": 1, "stepsDates": 1, "stepsEnables": 1, "tags": 1,
            "filter": 1
          }
        },
        function(err, doc) {
          ...
          if (groupStage==2) { // Lab is approved
            algoliaIndex.getObject(groupId, (err, content) => {
              if (content) {
                const stepsDates = data.stepsDates ? data.stepsDates : [null, null, null, null, null, null, null, null, null];
                ...
                if (content.positions) content.positions.push(object); else content.positions=[object];
                algoliaIndex.partialUpdateObject({
                  objectID: groupId,
                  positions: content.positions
                }, (err, content) => {
                  cb(err, userStage, userName, userEmail, hash, groupIndex, groupStage, userId, data.standout, data.filter, pro, interview, club);
                });
              } else {
                cb(err, userStage, userName, userEmail, hash, groupIndex, groupStage, userId, data.standout, data.filter, pro, interview, club);
              }
    ...
    // 5. send Email
    function (userStage: number, userName: string, userEmail: string, hash: string, groupIndex: groupComplex, groupStage: number, userId: string, standout: boolean, filter: boolean, pro: boolean, interview: boolean, club: boolean, cb) {
      var host: string;
      var type: number;
      ...
      if (userStage>0) { // User Exist
        if (groupStage==2) { // Lab Exist && Approved
          type = 0; console.log('confirmJobEmail')
        } else { // New Lab
          type = 1; console.log('verificationJobEmail') // emails.verificationJobEmail
        }
      } else { // User Not Exist
        type = 2; console.log('signupJobEmail') // emails.signupJobEmail
      }

      emails.confirmJobEmail(type,
                             host,
                             hash,
                             itemId,
                             groupLink,
...
export function post_position(userId: string, parentId: string, positionId: string, mode: number, callback) {
  async.waterfall([
    ...
    function (cb) {
      db.positions.updateOne(
         ...
         algoliaIndex.getObject(parentId, (err, content) => {
           const index = content ? content.positions.findIndex(y => y._id == positionId) : -1;
           if (index>-1) {
...
export function post_general(parentId: string, positionId: string, data, callback) {
  ...
  async.waterfall([
    ...
    function (cb) {
      db.positions.updateOne(
        ...
        algoliaIndex.getObject(parentId, (err, content) => {
          const index = content.positions.findIndex(y => y._id == positionId);
          if (index>-1) {
            content.positions[index].spotsAvailable = data.spotsAvailable;
            content.positions[index].contractLength = data.contractLength;
            content.positions[index].contractLengthType = data.contractLengthType;
            algoliaIndex.partialUpdateObject({
              objectID: parentId,
              positions: content.positions
            }, (err, content) => {
...
export function post_deadlines(positionId: string, parentId: string, data, callback) {
...
export function delete_position(groupId: string, userId: string, positionId: string, callback) {
  ...
  async.series({
      ...
      .next().then(function(item) {
        ...
        async.parallel({
          ...
          delete_news: function (cb) {
            news.add_group_activity(userId, 1105, positionId, groupId, positionId, [], null, null, null, 3, function (err) {
              cb(err)
            })
          },
    ...
    followers: function (cb) {
      peoples.delete_followings_ids("positions", "positionsIds", positionId, true, function (err, result) {
        cb(err, result)
      });
    },
    ...
    position: function (cb) {
      db.positions.deleteOne({ _id: ObjectID(positionId) },
                            { safe: true },
                            cb());
    },
    ...
    algolia: function (cb) {
      algoliaIndex.getObject(groupId, (err, content) => {
        if (content) {
          const positions = content.positions.filter(r => r._id!=positionId);
          algoliaIndex.partialUpdateObject({
            objectID: groupId,
            positions: positions
          }, (err, content) => {
...
function createPositionDocument(data, groupId: string, group: groupComplex, country: string, state: string, city: string, location: number[], adminFlag: boolean, callback) {
  const stepsDates = data.stepsDates ? data.stepsDates : [null, null, null, null, null, null, null, null, null];
  const stepsEnables = data.stepsEnables ? data.stepsEnables : [false, false];
  ...
  db.positions.insertOne(
    {
      ...
      "projects": data.projects ? data.projects.map(r => {r._id = ObjectID(r._id); return r}) : [],
...
export function positions_list(positionsIds: string[], userId: string, mini: number, aiStatus: number, adminFlag: boolean, callback) {
  ...
  var a = { "$addFields" : { "__order" : { "$indexOfArray" : [ positionsIds, "$_id" ] } } };
  ...
  if (mini==0) {
    ...
    f = { "$project" :
          {
           _id: 1, views: 1, groupId: 1, created_on: 1, mode: 1, payment: 1, standout: 1,
           title: 1, type: 1, position: 1, internalId: 1, description: 1, spotsAvailable: 1,
           note: 1, stepsDates: 1, stepsEnables: 1,
           gradesRequired: 1, lettersRequired: 1, lettersGuidelines: 1, numReferees: 1,
           apply: {
                   $filter: {
                      input: "$apply",
                      as: "apply",
  ...
  } else if (mini==2) { // Algolia object

    f = { "$project" :
          {
            _id: 1, mode: 1, standout: 1, position: 1, positionName: 1,
            title: 1, type: 1, description: 1, spotsAvailable: 1,
            created_on: 1, stepsDates: 1, stepsEnables: 1,
            contractLength: 1, contractLengthType: 1, tags: 1
            // grades, letters, referees
          }
        };
  };

  db.positions.aggregate( [ m, a, s, f ] ).map(
   function(u) {
     if (adminFlag==false) {
       delete u.payment;
       delete u.mode;
...
export function position_details(positionId: string, adminFlag: boolean, userId: string, callback) {
  var m = {
            "$match": {
              "_id": ObjectID(positionId),
              "payment": (adminFlag==true) ? {$in: [false, true]} : {$in: [true]},
  ...
  var p = {
           _id: 1, groupId: 1, mode: 1, how: 1, direct: 1,
           position: 1, title: 1, type: 1, site: 1, style: 1, tags: 1, projects: 1,
           created_on: 1, views: 1, standout: 1,
           contractLength: 1, contractLengthType: 1,
           spotsAvailable: 1, salary: 1, salaryCurrency: 1, internalId: 1,
           payment: 1,
           // hours: 1,
           stepsDates: 1, stepsEnables: 1,
           gradesRequired: 1, lettersRequired: 1, lettersGuidelines: 1, numReferees: 1,
           description: 1, duties: 1, scholarship: 1,
           expectations: 1, requiredEducation: 1, requiredExperience: 1,
           apply: {
                   $filter: {
                      input: "$apply",
                      as: "apply",
  ...
  db.positions.aggregate( [ m, f ] ).map(
   function(u) {
     if (adminFlag==false) {
       delete u.payment;
     }
...
export function position_stats(positionId: string, callback) {
  ...
  db.positions.aggregate( [ m, f ] ).next().then(function(item) {
...
export function put_apply(userId: string, mode: number, positionId: string, data: OpenPositionApply, callback) {
  var data_clone;
  ...
  async.waterfall([
    ...
    function (cb) {
      var m = { "$match": { "_id": ObjectID(positionId) } };
      var f = { "$project": { _id: 1, filter: 1, stepsDates: 1, apply: { $filter: { input: "$apply", as: "apply", cond: { $eq: [ "$$apply.id", userId ] } } } } };
      db.positions.aggregate( [ m, f ] ).next().then(function(item) {
        ...
        } else if (item.stepsDates[0]!=null && moment().isAfter(item.stepsDates[0])) {
          console.log("APPLICATION DEADLINE");
          cb(backhelp.invalid_apply())
        } else {
          // const filter: number = item.filter ? 1 : 0;
          if (mode==0) { // Academig Apply
            db.positions.updateOne(
              { _id: ObjectID(positionId) },
              { $push: {
                "apply": {
...
export function post_apply(userId: string, positionId: string, mode: number, data: OpenPositionApply, callback) {
  var data_clone;
...
export function post_candidate_note(positionId: string, peopleId: string, note: string, callback) {
  db.positions.updateOne(
...
export function post_candidate_stage(userId: string, positionId: string, peopleId: string, newStage: number, callback) {
  async.waterfall([
    ...
    function (cb) {
      db.positions.findOneAndUpdate(
        ...
        function(err, doc) {
          cb(err, doc.value.apply[0].stage, doc.value.apply[0].status, doc.value.stepsDates[7]);
        }
      );
    },
    ...
    // 3. push Notification
    function (cb) {
      application_notify(userId, peopleId, positionId, 1, newStage, function (err) {
        cb(err)
      })
    }
...
function push_news(userId: string, positionId: string, groupId: string, description: string, spotsAvailable: number, stepsDates, callback) {
  if (stepsDates) {
    var text = spotsAvailable + " spots available.";
    text += stepsDates[0] ? (" Application submission deadline is " + stepsDates[0] + ".") : "";
    text += stepsDates[6] ? (" Start date is " + stepsDates[8] + ".") : "";
  } else {
    text = '';
  }

  news.add_group_activity(userId, 1005, positionId, groupId, positionId, [], description, text, null, 3, function (err) {
    callback(err)
  })
}

function application_notify(userId: string, peopleId: string, positionId: string, type: number, mode: number, callback) {
  ...
  async.waterfall([
    ...
    function (cb) {
      ...
      const p = (type==0) ? {_id: 0, groupId: 1, title: 1, positionName: 1, "apply.id": 1} : {_id: 0, groupId: 1, title: 1, positionName: 1}
      db.positions
        .find({ _id: ObjectID(positionId) })
        .project(p)
      .next().then(function(item) {
        if (type==0 && item.apply) candidatesIds = item.apply.map(r => r.id);
        cb(null, item.groupId, item.title, item.positionName, candidatesIds)
      })
    },

    // 2. retrieve groupIndex, IDs of all admins
    function (groupId: string, positionTitle: string, positionName: string, candidatesIds: string[], cb) {
      if (type==1) {
        cb(null, positionTitle, positionName, candidatesIds, null, null)
      } else {
        groups.get_group_link(groupId, function (err, item) {
          cb(err, positionTitle, positionName, candidatesIds, item.groupIndex, item.peoplesItems.activesIds)
        })
      }
    },

    // 3. retrieve users objects
    function (positionTitle: string, positionName: string, candidatesIds: string[], groupIndex: groupComplex, activesIds: string[], cb) {
      var usersIds: string[];

      if (type==0) { // retrieve users of all candidates
        usersIds = candidatesIds;
      } else if (type==1) { // retrieve user of peopleId (candidate)
        usersIds = [peopleId];
      } else if (type==2) { // retrieve user of userId (candidate)
      ...
      db.peoples.aggregate( [ m, a, s, f ] ).toArray().then(function(users) {
        var toEmails: string[];
        if (type==2) {
          users=users.filter(r => (r._id==userId || (r.positions[0] && r.positions[0].status>4)))
          toEmails = users.slice(0,-1).map(r=>r.personalInfo.email);
        } else {
          toEmails = users.map(r=>r.personalInfo.email);
        }
        cb(null, positionTitle, positionName, candidatesIds, groupIndex, users, toEmails)
      })
    },

    // 4. send Email
    function (positionTitle: string, positionName: string, candidatesIds: string[], groupIndex: groupComplex, users: objectMiniEmail[], toEmails: string[], cb) {
      ...
      switch (type) {
        case 0: // Notify all candidates
          ...
          switch (mode) {
            case 0: // On Hold XXX
              msg = "Job posting is now on hold"; break;
            case 1: // Resume XXX
              msg = "Job posting is resumed"; break;
            case 2: // Cancel XXX
              msg = "Job posting canceled"; break;
            case 3: // Submission date
              msg = "Submission deadline is today"; break;
            case 4: // Decision date
              msg = "Decision date is today"; break;
            case 5: // Start date
              msg = "Start date is today"; break;
          };
          break
        case 1: // Notify candidate
          ...
          switch (mode) {
            case 0: // Shortlist XXX
              msg = "Congratulations, your application have been shortlisted.";  break;
            case 1: // Waitlist XXX
              msg = "Your application have been moved to the waitlist."; break;
            case 2: // Archived XXX
              msg = "Your application was declined."; break;
          };
          break;
        case 2: // Notify admins
          ...
          switch (mode) {
            case 0: // Candidate apply XXX
              msg = "New application: " + users[users.length-1].name; break;
            case 2: // Candidate post application XXX
              msg = null; break;
            case 3: // Candidate withdraw XXX
              msg = "Application withdraw " + users[users.length-1].name; break;
          };
          break;
      }
      if (toEmails.length>0) {
        emails.applicationJobEmail(subject, groupIndex.group.link, groupIndex.group.name, positionId, positionTitle, positionName, msg, users, toEmails, function (err) {
          cb(err)
        })
      } else {
        cb()
...
function invalid_position_name() {
    return backhelp.error("invalid_position_name", "Project names can have letters, #s, _ and, -");
}
