...
const algoliaIndex = (process.env.PORT) ? client.initIndex('services') : client.initIndex('dev_services');
...
export function put_resource(userId: string, data, insertIndex: number, adminFlag: boolean, callback) {
  var data_clone;
  ...
  async.waterfall([
    ...
    function (resource_data, cb) {
      ...
      createResourceDocument(data_clone, adminFlag, function (err, resourceId) {
    ...
    function (resourceId, cb) {
      ...
      if (adminFlag && data_clone.ai) {
        ...
        push_news(userId, resourceId, data_clone.groupId, data_clone.description, peoplesIds, function (err) {
          cb(err, resourceId)
        })
      }
    }
...
export function post_resource_notify(stripeId: string, subscriptionId: string, itemId: string, callback) {
  ...
  async.waterfall([
    ...
    function (cb) {
      groups.group_notify_details(stripeId, function (err, groupIndex, country, state, city, location, stage, userId, pro, interview, club) {
        cb(err, groupIndex, country, state, city, location,  stage, userId, pro, interview, club);
      })
    },

    // 2. get hash associated with User ID
    function (groupIndex: groupComplex, country: string, state: string, city: string, location: string[], groupStage: number, userId: string, pro: boolean, interview: boolean, club: boolean, cb) {
      db.hash
        .find({"userId": userId})
        .project({hash: 1})
      .next().then(function(item) {
        cb(null, item ? item.hash : null, groupIndex, country, state, city, location, groupStage, userId, pro, interview, club);
      });
    },

    // 3. retrieve User Email, Name
    function (hash: string, groupIndex: groupComplex, country: string, state: string, city: string, location: string[], groupStage: number, userId: string, pro: boolean, interview: boolean, club: boolean, cb) {
      db.peoples
        .find({"_id": userId})
        .project({stage: 1, name: 1, "personalInfo.email": 1})
      .next().then(function(item) {
        cb(null, item.stage, item.name, item.personalInfo.email, hash, groupIndex, country, state, city, location, groupStage, userId, pro, interview, club);
      });
    },

    // 4. retrieve Resource data && set payment_flag=true
    function (userStage: number, userName: string, userEmail: string, hash: string, groupIndex: groupComplex, country: string, state: string, city: string, location: string[], groupStage: number, userId: string, pro: boolean, interview: boolean, club: boolean, cb) {
      ...
      db.resources.findOneAndUpdate(
        ...
        { projection: { "created_on": 1, "mode": 1, "standout": 1, "name": 1, "pic": 1, "categoryId": 1, "description": 1, "tags": 1, "price": 1 } },
        function(err, doc) {
          ...
          if (groupStage==2) { // Lab is approved
            ...
            var object = {
              ...
              state: state,
              city: city,
              _geoloc: (location && location[0]) ? { lat: location[0], lng: location[1] } : { lat: '', lng: '' }
            }
            if (data.standout==2) object["pin"] = 1;
            algoliaIndex.addObjects([object], (err, content) => {
              cb(err, userStage, userName, userEmail, hash, groupIndex, groupStage, userId, data.standout, pro, interview, club);
            });
          } else {
            cb(err, userStage, userName, userEmail, hash, groupIndex, groupStage, userId, data.standout, pro, interview, club);
          }
    ...
    // 5. send Email
    function (userStage: number, userName: string, userEmail: string, hash: string, groupIndex: groupComplex, groupStage: number, userId: string, standout: boolean, pro: boolean, interview: boolean, club: boolean, cb) {
      var host: string;
      var type: number;
      ...
      if (userStage>0) { // User Exist
        if (groupStage==2) { // Lab Exist && Approved
          type = 0; console.log('confirmServiceEmail')
        } else { // New Lab
          type = 1; console.log('verificationServiceEmail') // emails.verificationJobEmail
        }
      } else { // User Not Exist
        type = 2; console.log('signupServiceEmail') // emails.signupJobEmail
      }

      emails.confirmServiceEmail(type,
                                 host,
                                 hash,
                                 itemId,
                                 groupLink,
...
export function post_resource(userId: string, parentId: string, resourceId: string, mode: number, callback) {
  async.waterfall([
...
export function delete_resource(parentId: string, userId: string, resourceId: string, category: number, type: number, callback) {
  ...
  async.series({
        ...
        db.groups.aggregate([
          ...
          db.groups.updateOne(
            { _id: ObjectID(parentId)},
            {
              ...
              $pull: { ['resourcesItems.resourcesIds']: ObjectID(resourceId) } ,
            },
            { multi: false, safe: true },
            cb()
          )
    ...
    items: function (cb) {
      db.resources
        ...
        .project({
          "_id": 0, "subId": 1, "backgroundPic": 1, "gallery": 1, "people": 1, "projects": 1, "faqsIds": 1, "publicationsIds": 1, "followedIds": 1,
          "manuals": 1, "codes": 1, "cads": 1, "inventories": 1, "equipments": 1, "requests": 1
        })
      .next().then(function(item) {
        ...
        async.parallel({
          ...
          delete_news: function (cb) {
            if (type==0) {
              ...
              news.add_group_activity(userId, 1102, resourceId, parentId, resourceId, peoplesIds, null, null, null, 3, function (err) {
                cb(err)
              })
            } else {
              cb()
          ...
          pics: function (cb) {
            pics.delete_pic_direct(item.backgroundPic, function(err) {
              pics.delete_pic_gallery((item.gallery[0] || {}).pic, function(err) {
                cb(err)
              })
            })
          },
          ...
          manuals: function (cb) {
            async.forEachOf(item.manuals, function (manual, key, cb) {
          ...
          codes: function (cb) {
            async.forEachOf(item.codes, function (code, key, cb) {
          ...
          cads: function (cb) {
            async.forEachOf(item.cads, function (cad, key, cb) {
          ...
          inventories: function (cb) {
            async.forEachOf(item.inventories, function (inventory, key, cb) {
              pics.delete_pic_direct(inventory.pic, function(err) {
                pics.delete_pic_gallery(inventory.files, function(err) {
                  cb(err)
                })
              })
            }, function (err) {
          ...
          equipments: function (cb) {
            async.forEachOf(item.equipments, function (equipment, key, cb) {
              pics.delete_pic_direct(equipment.pic, function(err) {
                pics.delete_pic_gallery(equipment.files, function(err) {
                  cb(err)
                })
              })
            }, function (err) {
          ...
          requests: function (cb) {
            async.forEachOf(item.requests, function (request, key, cb) {
              // FIX: pull "requests" only from request.userId
              const ids: string[] = [request.userId].concat(item.people.map(r => r._id));
          ...
          stripe: function (cb) {
            stripe.subscriptions.del(
              ...
              function(err, confirmation) {
                console.log('delete_resource_subscription',err,confirmation)
                cb(err)
              }
            );
          }
    ...
    followers: function (cb) {
      peoples.delete_followings_ids("resources", "resourcesIds", resourceId, true, function (err) {
        cb(err)
      });
    },
    ...
    resource: function (cb) {
      db.resources.deleteOne({ _id: ObjectID(resourceId) },
                             { safe: true },
                             cb());
    },
...
export function resource_typeahead_ids(term: string, existResourcesIds: string[], callback) {
  db.resources
...
export function resources_list(resourcesIds: string[], followingsIds: string[], userId: string, mini: number, aiStatus: number, adminFlag: boolean, callback) {
  ...
  var a = { "$addFields" : { "__order" : { "$indexOfArray" : [ resourcesIds, "$_id" ] } } };
  ...
  if (mini==0) {
    ...
    f = {
          "$project" : {
            _id: 1, name: 1, pic: 1, tags: 1, created_on: 1, mode: 1, payment: 1, standout: 1,
            description: 1, price: 1, groupId: 1, profileId: 1, views: 1, categoryId: 1
          }
        };
    ...
    f = {
          "$project" : {
            _id: 1, name: 1, pic: 1, created_on: 1, tags: 1, description: 1,
            price: 1, groupId: 1, profileId: 1, views: 1, categoryId: 1,
            requests: {
                    $filter: {
                       input: "$requests",
                       as: "request",
  ...
  if (followingsIds && (mini==0 || mini==1 || mini==2)) {
    ...
    db.resources.aggregate( [ m, a, s, f ] ).map(
     function(u) {
       if (adminFlag==false) {
         delete u.payment;
         delete u.mode;
...
export function resource_details(resourceId: string, followingsIds: string[], adminFlag: boolean, userId: string, callback) {
  const m = {
              "$match" : {
                "_id" : ObjectID(resourceId),
                "payment": (adminFlag==true) ? {$in: [false, true]} : {$in: [true]},
  ...
  const f = {
             "$project" : {
                           _id: 1, name: 1, pic: 1, categoryId: 1, payment: 1, mode: 1,
                           groupId: 1, profileId: 1, standout: 1,
                           how: 1, direct: 1,
                           created_on: 1, views: 1, downloads: 1, followedIds: 1,
                           people: 1, projects: 1, tags: 1, price: 1,
                           description: 1, descriptionCaption: 1, descriptionPic: 1,
                           termsMode: 1, termsMore: 1, publicationsIds: 1,
                           gallery: 1, links: 1, faqsIds: 1,
                           manuals: 1, codes: 1, cads: 1, inventories: 1, equipments: 1,
                           requests: {
                                      $filter: {
                                        input: "$requests",
                                        as: "request",
  ...
  if (followingsIds) {
    ...
    db.resources.aggregate( [ m, f ] ).map(
     function(u) {
       if (adminFlag==false) {
         delete u.payment;
       }
...
export function resource_items_ids(mode, resourceId, callback) {
  ...
  switch (mode) {
    ...
    case 1: key = "faqsIds"; break;
    case 2: key = "people"; break;
  }
...
function push_news(userId: string, resourceId: string, groupId: string, description: string, peoplesIds: string[], callback) {
  news.add_activity(0, userId, 1002, resourceId, groupId, resourceId, peoplesIds, description, null, null, false, function (err) {
    groups.groupMembers(groupId, 3, function (err, peoples) {
      async.forEachOf(peoples.map(r => r._id), function (activeId, key, cb) {
        if (userId==activeId) {
          ...
          news.add_notification(userId, 9001, resourceId, activeId, groupId, peoplesIds, description, null, null, function (err) {
            cb()
          });
        } else {
          news.add_notification(userId, 1002, resourceId, activeId, groupId, peoplesIds, description, null, null, function (err) {
            cb()
          });
        }
      }, function (err) {
...
export function post_terms(termsMode: number, termsMore: string, itemId: string, callback) {
  db.resources.updateOne(
     {_id: ObjectID(itemId)},
     { $set:
       {
...
export function post_request(text: string, userId: string, resourceId: string, callback) {
  async.waterfall([
    ...
    function (peopleIds: string[], cb) {
      db.channels.findOne(
        { "users._id": { $all: peopleIds }, "mode": 2, parentId: ObjectID(resourceId)  }
      ).then(function(item) {
    ...
    function (peopleIds: string[], existChannelId: string, cb) {
      if (peopleIds) {
        messages.createChannelDocument([userId].concat(peopleIds), ObjectID(resourceId), 2, function (err, channelId) {
...
export function channel_items(resourceId, callback) {
  db.resources
    ...
    .project({ "requests": 1 })
  .next().then(function(item) {
    callback(null, item.requests.map(r => r.channelId))
  })
}
...
export function put_table(data, mode: number, resourceId: string, callback) {
  ...
  switch(mode) {
     case 0: key="manuals"; break;
     case 1: key="codes"; break;
     case 2: key="cads"; break;
     case 3: key="inventories"; break;
     case 4: key="equipments"; break;
...
export function delete_table(mode: number, itemId: string, resourceId: string, callback) {
  var key: string;
...
function invalid_resource_name() {
    return backhelp.error("invalid_resource_name",
