...
const algoliaIndex = process.env.PORT
  ? client.initIndex("mentors")
  : client.initIndex("dev_mentors");
...
export function mentor_details(
  mentorId: string,
  adminFlag: boolean,
  userId: string,
  callback
...
export function put_submit_mentor(
  userId: string,
  submitMentor: SubmitMentor,
  callback
) {
  var data_clone;
  ...
  async.waterfall(
    [
      ...
      function (submit_mentor_data, cb) {
        ...
        if (userId) {
          updateSubmitMentorDocument(
            userId,
            data_clone,
            function (err, mentorSubmitId) {
              cb(err, mentorSubmitId);
            }
          );
        } else {
          createSubmitMentorDocument(
            data_clone,
            function (err, mentorSubmitId) {
              cb(err, mentorSubmitId);
            }
...
export function post_availability(
...
export function post_expertise(
  expertise: Expertise,
  mentorId: string,
  type: number,
  callback
...
export function delete_expertise(
  id: string,
  mentorId: string,
  type: number,
  callback
...
export function channel_items(mentorId, callback) {
  db.mentors
    ...
    .then(function (item) {
      callback(
        null,
        item.sessions.map((r) => r.channelId)
      );
    });
}
