...
export function create_deal_payment(
  host: string,
  dealId: string,
  stripeId: string,
  priceId: string,
    ...
    if (type == 0) {
      ...
      if (type == 1) {
        ...
        switch (mode) {
          case 0:
            {
              ...
              if (process.env.PORT) {
                ...
                planId =
                  period == 0
                    ? "price_1I1z2wHfJudkfqK1JqMPO7ac"
                    : "price_1I1z2wHfJudkfqK1pQu72of5";
              } else {
                // DEVELOPMENT
                planId =
                  period == 0
                    ? "price_1HtxkWHfJudkfqK1Lc6LyBZd"
                    : "price_1HO9PPHfJudkfqK1b125VOSK";
              }
              trial_period_days = 7;
          ...
          case 1:
            {
              // Academig Lab Tools PRO
              if (process.env.PORT) {
                // PRODUCTION
                planId =
                  period == 0
                    ? "price_1I1z4pHfJudkfqK1l2354CNT"
                    : "price_1I1z4pHfJudkfqK1dnk2YJ88";
              } else {
                // DEVELOPMENT
                planId =
                  period == 0
                    ? "price_1HNmSpHfJudkfqK14vOHTD8v"
                    : "price_1HO9QKHfJudkfqK1Ics7Y4e3";
              }
        ...
        switch (mode) {
          case 0: {
            // Academig PRO & Mentors
            if (process.env.PORT) {
              // PRODUCTION
              planId =
                period == 0
                  ? "price_1I1z3nHfJudkfqK166yHhW81"
                  : "price_1I1z3nHfJudkfqK19u5r8nl1";
            } else {
              // DEVELOPMENT
              planId =
                period == 0
                  ? "price_1HxAZIHfJudkfqK1vcz2dp5d"
                  : "price_1HxAYmHfJudkfqK1VcLccH7f";
            }
            trial_period_days = 7;
            link = "http://" + host + "/search";
            break;
          ...
          case 1: {
            // Academig Labs Suggestions PRO
            if (process.env.PORT) {
              // PRODUCTION
              planId =
                period == 0
                  ? "price_1I1z4jHfJudkfqK1mWXrfBcV"
                  : "price_1I1z4jHfJudkfqK1JaZLw9Yw";
            } else {
              // DEVELOPMENT
              planId =
                period == 0
                  ? "price_1HNmSQHfJudkfqK1BHbKORzl"
                  : "price_1HO9RRHfJudkfqK1ahPGjf9H";
            }
            trial_period_days = 7;
            link =
              "http://" +
...
export function create_position_subscription(
  userId: string,
  host: string,
  build: number,
  filter: number,
  ...
  if (userId) {
    ...
    success_url =
      "http://" + host + "/post-job/confirm/" + itemId + "#" + groupEmail;
    ...
    if (filter)
      planItems.push({
        plan: process.env.PORT
          ? "price_1I1z5aHfJudkfqK15lKLMqqf"
          : "price_1HfrdIHfJudkfqK1yFoGGHcb",
      });
    if (build)
      planItems.push({
        plan: process.env.PORT
...
export function create_service_subscription(
  userId: string,
  host: string,
  build: number,
  standout: number,
  ...
  if (userId) {
    ...
    // Non-User
    success_url =
      "http://" + host + "/post-service/confirm/" + itemId + "#" + groupEmail;
...
export function lab_pro_subscribe(
  stripeId: string,
  subscriptionId: string,
  callback
) {
...
export function ai_pro_subscribe(
  stripeId: string,
  subscriptionId: string,
  callback
) {
    ...
    if (customer) {
      ...
      customer.subscriptions.data.forEach((sub, index) => {
        ...
        if (process.env.PORT) {
          // PRODUCTION
          switch (plan.product) {
            case "prod_IdFZsOCsSSes7g":
              planName = 1;
              break; // Tools PRO
            case "prod_IdFZfzkmnXdgNm":
              planName = 2;
              break; // AI PRO
            default:
              planName = 3;
          ...
          switch (plan.product) {
            case "prod_HxhsVnODfiqGaX":
              planName = 1;
              break; // Tools PRO
            case "prod_HxhsnVwXmakwxT":
              planName = 2;
              break; // AI PRO
            default:
              planName = 3;
