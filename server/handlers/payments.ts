  ...
  async.waterfall([
    ...
    function (stripeId: string, cb) {
      ...
      stripe.billingPortal.sessions.create(
        {
          customer: stripeId,
          return_url: (mode==0) ? 'https://www.academig.com/settings/account' : 'https://www.academig.com'
  ...
  async.waterfall([
    ...
    function (userId: string, cb) {
      if (mode==0) {
        ...
        group_data.group_stripe_id(getp.id, function (err, stripeId, groupIndex, groupEmail) {
    ...
    function (stripeId: string, groupIndex: groupComplex, cb) {
      payments_data.create_source(host, mode, stripeId, groupIndex, function (err, payment_info) {
        cb(err, payment_info);
      })
    }
...
exports.dealPost = function (req, res) {
  ...
  async.waterfall([
    ...
    function (stripeId: string, priceId: string, dealId: string, cb) {
      ...
      payments_data.create_deal_payment(host, dealId, stripeId, priceId, quantity, function (err, session, planId) {
        cb(err, session)
      })
    }
...
exports.subscribePost = function (req, res) {
  ...
  async.waterfall([
    ...
    // 1. make sure Mode, Period, Type, Parent ID Are valid
    function (cb) {
      if (mode==null || period==null || type==null || (!ObjectID.isValid(getp.id) && mode==1)) {
        cb(helpers.no_such_item());
        return;
      }
      cb(null);
    ...
    function (cb) {
      console.log('SU2')
      privilege_data.privilages(req.user.sub, getp.id, (mode==1 || mode==2) ? 0 : 10, function (err, userId, flag) {
        if (flag) cb(err, userId); else cb(helpers.invalid_privileges());
      })
    },
    ...
    function (stripeId: string, groupIndex: groupComplex, cb) {
      console.log('SU4')
      payments_data.create_subscription(host, mode, period, type, quantity, stripeId, groupIndex, function (err, session, planId) {
        cb(err, session)
      })
    }
...
exports.positionPost = function (req, res) {
  ...
  async.waterfall([
    ...
    function (cb) {
      console.log('PP1','standout',standout,'filter',filter,'build',build,'getp.itemId',getp.itemId,'getp.id',getp.id)
      if (standout==null || filter==null || build==null || !ObjectID.isValid(getp.itemId) || !ObjectID.isValid(getp.id)) {
        cb(helpers.no_such_item());
        return;
      }
      cb();
    ...
    function (cb) {
      ...
      if (req.user) {
        ...
        privilege_data.offline_build_privilages(getp.userId, getp.id, function (err, flag) {
    ...
    function (userId: string, stripeId: string, groupIndex: groupComplex, groupEmail: string, cb) {
      console.log('PP4')
      payments_data.create_position_subscription(userId, host, build, filter, standout, stripeId, groupIndex, groupEmail, getp.itemId, function (err, confirmation) {
        cb(err, confirmation)
      })
    }
...
exports.servicePost = function (req, res) {
  ...
  async.waterfall([
    ...
    // 1. make sure Standout, Filter, Build, Item ID, Parent ID are valid
    function (cb) {
      console.log('SS1','standout',standout,'build',build,'getp.itemId',getp.itemId,'getp.id',getp.id)
      if (standout==null || build==null || !ObjectID.isValid(getp.itemId) || !ObjectID.isValid(getp.id)) {
        cb(helpers.no_such_item());
        return;
      }
      cb();
    ...
    function (userId: string, stripeId: string, groupIndex: groupComplex, groupEmail: string, cb) {
      console.log('SS4')
      payments_data.create_service_subscription(userId, host, build, standout, stripeId, groupIndex, groupEmail, getp.itemId, function (err, confirmation) {
        cb(err, confirmation)
      })
    }
...
exports.payment = function (req, res) {
  ...
  async.waterfall([
    ...
    function (cb) {
      if (mode==null || (!ObjectID.isValid(getp.id) && (mode==1 || mode==2))) {
        cb(helpers.no_such_item());
        return;
      }
      cb(null);
...
exports.plan = function (req, res) {
  ...
  async.waterfall([
    ...
    function (stripeId: string, cb) {
      if (stripeId) {
        if (mode==0) {
          payments_data.user_plan(stripeId, mode, function (err, subscriptions) {
    ...
    function (stripeId: string, subscriptions, cb) {
      if (stripeId) {
        payments_data.user_billing_period(stripeId, function (err, period_end) {
          const plan = {
